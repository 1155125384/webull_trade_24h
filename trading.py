...
for i in range(1, 15):
    ...
    tickers_can_be_sell = [
        ...
        if float(item.get("unrealized_profit_loss_rate", 0)) > 0.003 and (item["symbol"] != "AAPL" and item["symbol"] != "FUTU" and item["symbol"] != "TTWO")
    ]
    ...
    print("-"*50)
    print(f"Completed for {i} time!")

    wait_time=60
    # Check if 'i' is odd or even
    if i % 2 != 0:
        # Odd Number: 1, 3, 5...
        print(f"Wait for 60s for next time transaction (Odd iteration).")
    else:
        # Even Number: 2, 4, 6...
